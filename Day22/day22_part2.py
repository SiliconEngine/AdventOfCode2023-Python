...
def count_falls_recurse(brick, level = 0):
    global counted

    if brick.id in counted:
        return 0
    counted.add(brick.id)
    count = 1
    brick.falling = True

    for supp in brick.supporting:
        solid_count = 0
        for under in supp.under_list:
            solid_count += not under.falling

        if solid_count == 0:
            count += count_falls_recurse(supp, level+1)

    return count

def count_falls(brick_list, brick):
    global counted
    counted = set()

    # Reset all the 'falling' flags
    for b in brick_list:
        b.falling = False

    count = count_falls_recurse(brick, 0)
    # Needs -1 because we don't count the first brick, because "disintegrated"
    return count - 1
...
def main():
    ...
    # Calculate how many bricks will fall for each brick
    total = 0
    for brick in brick_list:
        count = count_falls(brick_list, brick)
        total += count
